...
func (t *ArchiveDownloadTask) RunWithoutPushUploadTask() (*ArchiveContentUploadTask, error) {
	...
	dir, err := os.MkdirTemp(conf.Http.TempDir, "dir-*")
	if err != nil {
		return nil, err
	}
...
func genTempFileName(prefix string) (string, error) {
	retry := 0
	for retry < 10000 {
		newPath := stdpath.Join(conf.Http.TempDir, prefix+strconv.FormatUint(uint64(rand.Uint32()), 10))
